...
void setup()
{
  ...
  lcd.println(F("RAWX Logger F9P"));  //custom Sky Horse
  lcd.println(F("Logs GNSS data."));  //custom Sky Horse
  lcd.print(F("Interval minutes: "));  //custom Sky Horse
  lcd.println(F(INTERVAL));  //custom Sky Horse
  ...
  lcd.clear(); //custom Sky Horse
  lcd.println(F("Initializing GNSS..."));  //custom Sky Horse
  ...
  if (i2cGPS.begin(Wire,0x42) == false) //Connect to the Ublox module using Wire port
  {
    ...
    lcd.clear(); //custom Sky Horse
    lcd.println("Error!! Ublox GNSS not detected!");  //custom Sky Horse
  ...
  lcd.clear(); //custom Sky Horse
  lcd.println("Ublox GNSS found!");  //custom Sky Horse
  ...
  if (response == false) {
    ...
    lcd.clear(); //custom Sky Horse
    lcd.println(F("Unable to initialize GNSS!"));  //custom Sky Horse
    
    Serial.println("Waiting for reset...");
    lcd.println(F("Waiting for reset..."));  //custom Sky Horse
  ...
  if (digitalRead(modePin) == LOW) {
    ...
    lcd.clear(); //custom Sky Horse
    lcd.println("BASE mode selected");  //custom Sky Horse
    ...
    lcd.clear(); //custom Sky Horse
    lcd.println("ROVER mode selected");  //custom Sky Horse
  ...
  lcd.println("GNSS initialized!");  //custom Sky Horse
  ...
  lcd.clear(); //custom Sky Horse
  lcd.println("Initializing SD card...");  //custom Sky Horse
  // See if the SD card is present and can be initialized
  if (!sd.begin(cardSelect, SD_SCK_MHZ(4))) {
    Serial.println("Panic!! SD Card Init failed, or not present!");
    Serial.println("Waiting for reset...");
    lcd.clear(); //custom Sky Horse
    lcd.println("SD Card Init failed.");  //custom Sky Horse
    lcd.println("Waiting for reset...");  //custom Sky Horse
  #ifndef NoLED
    #ifdef NeoPixel
      setLED(red); // Set NeoPixel to red
    #endif
  ...
  delay(1000); //custom Sky Horse
  lcd.clear(); //custom Sky Horse
  lcd.println("SD Card initialized!");  //custom Sky Horse
  ...
  delay(1000); //custom Sky Horse
  lcd.clear(); //custom Sky Horse
  lcd.println("Waiting for GNSS fix...");  //custom Sky Horse
}
  ...
  switch(loop_step) {
    case init: {
      ...
      if (valfix == maxvalfix) { // wait until we have enough valid fixes
        ...
        if (vbat < LOWBAT) {
          ...
          delay(1000); //custom Sky Horse
          lcd.clear(); //custom Sky Horse
          lcd.println("Low Battery!");  //custom Sky Horse
          break;
        }
        ...
        if (base_mode == true) {
          if (digitalRead(SurveyInPin) == LOW) {
           ...
           // delay(1000); //custom Sky Horse
            lcd.clear(); //custom Sky Horse
            lcd.println("SURVEY_IN mode selected");  //custom Sky Horse
    ...
    case open_file: {
      ...
      if (rawx_dataFile.open(rawx_filename, O_CREAT | O_WRITE | O_EXCL)) {
        ...
        lcd.clear(); //custom Sky Horse
        lcd.print("Logging to ");  //custom Sky Horse
        lcd.println(rawx_filename);  //custom Sky Horse
      }
      // if the file isn't open, pop up an error:
        ...
        // delay(1000); //custom Sky Horse
        lcd.clear(); //custom Sky Horse
        lcd.println("Error opening RAWX file!");  //custom Sky Horse
        lcd.println("Waiting for reset...");  //custom Sky Horse
#ifndef NoLED
#ifdef NeoPixel
      ...
      if (!rawx_dataFile.timestamp(T_CREATE, (RTCyear+2000), RTCmonth, RTCday, RTChours, RTCminutes, RTCseconds)) {
        ...
        // delay(1000); //custom Sky Horse
        lcd.clear(); //custom Sky Horse
        lcd.println("Could not set file create timestamp!");  //custom Sky Horse
      }
#else
    ...
    case write_file: {
      ...
      if (bufAvail > 0) {
        ...
        if (bufAvail > maxSerialBufferAvailable) {
          ...
          lcd.clear(); //custom Sky Horse
          lcd.print("Max bufAvail: ");  //custom Sky Horse
          lcd.println(maxSerialBufferAvailable);  //custom Sky Horse
        }
#endif  
        ...
        if (bufferPointer == SDpacket) {
          ...
          if (numBytes != SDpacket) {
            ...
            lcd.clear(); //custom Sky Horse
            lcd.print("SD write error! Write size was ");  //custom Sky Horse
            lcd.print(SDpacket);  //custom Sky Horse
            lcd.print(". ");  //custom Sky Horse
            lcd.print(numBytes);  //custom Sky Horse
            lcd.println(" were written.");  //custom Sky Horse
          }
#endif
        }
        ...
        switch (ubx_nmea_state) {
          case (looking_for_B5_dollar): {
            if (c == 0xB5) { // Have we found Sync Char 1 (0xB5) if we were expecting one?
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting Sync Char 0xB5 or an NMEA $ but did not receive one!");  //custom Sky Horse
          ...
          case (looking_for_62): {
            if (c == 0x62) { // Have we found Sync Char 2 (0x62) when we were expecting one?
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting Sync Char 0x62 but did not receive one!");  //custom Sky Horse
          ...
          case (looking_for_class): {
            ...
            if ((ubx_class != 0x02) and (ubx_class != 0x0d) and (ubx_class != 0x01)) {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting Class of 0x02 or 0x0d or 0x01 but did not receive one!");  //custom Sky Horse
          ...
          case (looking_for_ID): {
            ...
            if ((ubx_class == 0x02) and ((ubx_ID != 0x15) and (ubx_ID != 0x13))) {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting ID of 0x15 or 0x13 but did not receive one!");  //custom Sky Horse
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting ID of 0x03 but did not receive one!");  //custom Sky Horse
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Was expecting ID of 0x02 or 0x07 or 0x03 but did not receive one!");  //custom Sky Horse
          ...
          case (looking_for_checksum_B): {
            ...
            if ((ubx_expected_checksum_A != ubx_checksum_A) or (ubx_expected_checksum_B != ubx_checksum_B)) {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("UBX checksum error!");  //custom Sky Horse
          ...
          case (looking_for_asterix): {
            ...
            if (ubx_length > max_nmea_len) { // If the length is greater than max_nmea_len, something bad must have happened (sync_lost)
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("Excessive NMEA message length!");  //custom Sky Horse
          ...
          case (looking_for_csum2): {
            ...
            if ((nmea_csum1 != nmea_expected_csum1) or (nmea_csum2 != nmea_expected_csum2)) {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("NMEA checksum error!");  //custom Sky Horse
          ...
          case (looking_for_term1): {
            ...
            if (c != '\r') {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("NMEA CR not found!");  //custom Sky Horse
          ...
          case (looking_for_term2): {
            ...
            if (c != '\n') {
              ...
              lcd.clear(); //custom Sky Horse
              lcd.println("NMEA LF not found!");  //custom Sky Horse
    ...
    case new_file: {
      ...
      lcd.println("File closed!");  //custom Sky Horse
    ...
    case close_file: {
      ...
      if (!rawx_dataFile.timestamp(T_WRITE, (RTCyear+2000), RTCmonth, RTCday, RTChours, RTCminutes, RTCseconds)) {
        Serial.println("Warning! Could not set file write timestamp!");
        lcd.println("Warning! Could not set file write timestamp!");  //custom Sky Horse
      }
#else
      rawx_dataFile.timestamp(T_WRITE, (RTCyear+2000), RTCmonth, RTCday, RTChours, RTCminutes, RTCseconds);
#endif
      ...
      if (stop_pressed == true) {
        ...
        lcd.println("Battery must be low - waiting for it to recover...");  //custom Sky Horse
#ifndef NoLED
#ifdef NeoPixel
        setLED(red); // Set the NeoPixel to red
#else
